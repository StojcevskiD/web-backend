create
    or replace function create_user_if_not_exists(_email text, _password text, _role text, _name text, _surname text,
                                                  _enabled bool) returns void as
$$

begin
    if
        not exists(select id from auth_user.users where email = _email) then
        insert into auth_user.users(email, password, date_created, name, surname, username, enabled)
        values (_email, _password, now(), _name, _surname, _name, _enabled);
